function AchievementCard({ metric, description, index }) {
  ...
  return (
    <div className={`${bgClass} p-7 flex flex-col justify-between min-h-[160px] md:min-h-[180px]`}>
      <p
        className={`font-black text-[32px] md:text-[40px] leading-none tracking-tight mb-3 ${metricClass}`}
      >
        {metric}
      </p>
      <p className={`text-[13px] leading-snug ${isDark ? 'text-white/60' : 'text-[#1A1A1A]/60'}`}>
        {description}
      </p>
    </div>
  )
